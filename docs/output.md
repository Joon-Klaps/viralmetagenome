...
- **Taxonomic classification** - a stacked bar chart, similar to the read-level Kaiju/Kraken2 bar charts, of the final consensus clusters per sample coloured by the species of their best [MMseqs-search (annotation)](#mmseqs-search-annotation) hit. Only the five most abundant species get their own colour, the rest are grouped as "Other species" and clusters without a significant hit as "Unclassified". The "All clusters" view adds a grey "Not reconstructed" segment that completes each bar to the number of clusters the sample started with, so the plot shows both what was found and what was lost during coverage filtering and refinement.
- **Genome completeness** - a heatmap of the mean consensus genome completeness per sample and species. Completeness is CheckV's `completeness` estimate when CheckV ran; otherwise it is estimated as the fraction of the annotation hit's reference genome that the consensus spans, discounted by the ambiguous bases QUAST counted. An empty cell means the species was not reconstructed in that sample. Segmented viruses get a column per segment (`Influenza A virus | seg 4`), so a partially recovered multipartite genome reads as the segments it is missing rather than as one averaged number - see [annotation sequences](usage/databases.md#annotation-sequences) for the metadata fields this is read from.

Both plots are only shown when contig annotation data is available, i.e. when neither `--skip_consensus_annotation` nor `--skip_consensus_qc` is set.
