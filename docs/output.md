...
The report also includes a "Contig Taxonomy" stacked bar chart, similar to the read-level Kaiju/Kraken2 bar charts, showing the number of final consensus contigs/clusters assigned to each species/taxon per sample (aggregated from the [MMseqs-search (annotation)](#mmseqs-search-annotation) results, never rendered per-contig). Where available (i.e. CheckV was run, or QUAST wasn't skipped), a second switchable dataset shows the mean genome completeness per sample/taxon - from CheckV's `completeness` estimate if present, otherwise a proxy of `100% - ambiguous bases (%)` from QUAST. This chart is only shown when contig annotation data is available, i.e. when neither `--skip_consensus_annotation` nor `--skip_consensus_qc` is set.
...
- `multiqc/`
  ...
  - `multiqc_data/contig_taxonomy*.txt`: the data underlying the "Contig Taxonomy" bar chart - contig/cluster counts per sample and species/taxon, and (when available) mean completeness per sample and species/taxon.
